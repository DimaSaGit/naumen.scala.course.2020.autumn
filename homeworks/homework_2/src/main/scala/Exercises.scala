...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        ...
        return (l.toSet.toSeq);
    }

    def isNumberSimple(number: Int): Boolean = {
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        var l = new ListBuffer[(String, Double)];
        for ((name,(radius,density)) <- ballsArray) {

            l += ((name, getBallsVolume(radius) * density));
        }
        var answer = l.sortBy(_._2).toSeq;

        for{(material, weight)<-answer
            } yield material;

    }
    def getBallsVolume(radius:Int):Double={
        return scala.math.pow(radius,2)*scala.math.Pi;
    }
